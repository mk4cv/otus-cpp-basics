...
int main() {
    ...
    SequentialContainer<int> seq_сontainer; // Создание объекта контейнера для хранения int
    for (int i = 0; i < 10; ++i) {
        seq_сontainer.push_back(i);
    } // Добавление 10 элементов в конец
    printContainer(seq_сontainer, "После добавления 0..9"); // Вывод содержимого
    std::cout << "Размер контейнера: " << seq_сontainer.size() << "\n\n"; // Вывод размера
    // Удаление 3-го, 5-го и 7-го элементов (в обратном порядке)
    seq_сontainer.erase(6);  // Удаление элемента с индексом 6
    seq_сontainer.erase(4);  // Удаление элемента с индексом 4
    seq_сontainer.erase(2);  // Удаление элемента с индексом 2
    printContainer(seq_сontainer, "После удаления элементов на позициях 2, 4, 6"); // Вывод после удалений
    seq_сontainer.insert(0, 10); // Добавление элемента 10 в начало
    printContainer(seq_сontainer, "После вставки 10 в начало"); // Вывод содержимого
    seq_сontainer.insert(4, 20); // Добавление элемента 20 в середину (индекс 4)
    printContainer(seq_сontainer, "После вставки 20 в середину (индекс 4)"); // Вывод содержимого
    seq_сontainer.push_back(30); // Добавление элемента 30 в конец
    printContainer(seq_сontainer, "После добавления 30 в конец"); // Вывод содержимого

    // Тестирование однонаправленного спискового котнейнера
    std::cout << "\n=== 2. Тест однонаправленного спискового котнейнера ===\n\n";
    SinglyLinkedList<int> single_list; // Создание объекта контейнера для хранения int
    for (int i = 0; i < 10; ++i) {
        single_list.push_back(i);
    } // Добавление 10 элементов
    printContainer(single_list, "После добавления 0..9"); // Вывод содержимого
    // Удаление 3-го, 5-го и 7-го элементов (в обратном порядке)
    single_list.erase(6);
    single_list.erase(4);
    single_list.erase(2);
    printContainer(single_list, "После удалений"); // Вывод содержимого
    single_list.insert(0, 10); // Добавление элемента 10 в начало
    printContainer(single_list, "После вставки 10 в начало"); // Вывод содержимого
    single_list.insert(4, 20); // Добавление элемента 20 в середину (индекс 4)
    printContainer(single_list, "После вставки 20 в середину"); // Вывод содержимого
    single_list.push_back(30); // Добавление элемента 30 в конец
    printContainer(single_list, "После добавления 30 в конец"); // Вывод содержимого
    
    // Тестирование двунаправленного спискового котнейнера (доп. задапние №3)
    std::cout << "\n=== 3. Тест двунаправленного спискового котнейнера ===\n\n";
    DoublyLinkedList<int> doubleList; // Создание объекта контейнера для хранения int
    for (int i = 0; i < 10; ++i) {
        doubleList.push_back(i);
    } // Добавление 10 элементов
    printContainer(doubleList, "DoublyLinkedList после добавления 0..9");
    // Удаление 3-го, 5-го и 7-го элементов (в обратном порядке)
    doubleList.erase(6);
    doubleList.erase(4);
    doubleList.erase(2);
    printContainer(doubleList, "После удалений"); // Вывод содержимого
    doubleList.insert(0, 10); // Добавление элемента 10 в начало
    printContainer(doubleList, "После вставки 10 в начало"); // Вывод содержимого
    doubleList.insert(4, 20); // Добавление элемента 20 в середину (индекс 4)
    printContainer(doubleList, "После вставки 20 в середину"); // Вывод содержимого
    doubleList.push_back(30); // Добавление элемента 30 в конец
    printContainer(doubleList, "После добавления 30 в конец"); // Вывод содержимого
    
    // Тестирование семантики перемещения для последовательного контейнера
    std::cout << "\n=== 4. Тест семантики перемещения для последовательного контейнера ===\n\n";
    SequentialContainer<int> source_seq_сontainer; // Создание объекта контейнера для хранения int
    source_seq_сontainer.push_back(100); // Добавление элемента 100 в конец
    source_seq_сontainer.push_back(299); // Добавление элемента 200 в конец
    std::cout << "Источник до перемещения: ";
    for (auto val : {source_seq_сontainer[0], source_seq_сontainer[1]}) std::cout << val << " "; // Вывод содержимого
    std::cout << "\n"; 
    SequentialContainer<int> destination_seq_сontainer(std::move(source_seq_сontainer)); // Конструктор перемещения
    std::cout << "Приемник после перемещения:";
    for (size_t i = 0; i < destination_seq_сontainer.size(); ++i) {
        std::cout << destination_seq_сontainer[i] << " ";
    } // Вывод содержимого
    std::cout << "\n";
    std::cout << "Размер источника после перемещения (должен быть 0): " << source_seq_сontainer.size() << "\n\n";
    
    // Тестирование итераторов (доп. задание №4)
    std::cout << "=== 5. Тест итераторов для последовательного контейнера ===\n\n";
    SequentialContainer<int> seq_сontainer_iteration; // Создание объекта контейнера для хранения int
    for (int i = 0; i < 5; ++i) {
        seq_сontainer_iteration.push_back(i * 10);  // 0, 10, 20, 30, 40
    }  // Добавление 5 элементов
    std::cout << "Обход через итераторы (operator*):\n";
    for (auto iter = seq_сontainer_iteration.begin(); iter != seq_сontainer_iteration.end(); ++iter) {
        std::cout << *iter << " ";  // Разыменование
    } // Обход через итераторы operator*, вывод содержимого
    std::cout << "\n\n";
    
    std::cout << "Обход через итераторы (метод get()):\n";
    for (auto iter = seq_сontainer_iteration.begin(); iter != seq_сontainer_iteration.end(); ++iter) {
        std::cout << iter.get() << " ";  // Получение через метод get()
    } // Обход через итераторы методами get(), вывод содержимого
    std::cout << "\n";
